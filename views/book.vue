	...
	<div class="page" transition="expand">
		<div class="card">
							...
							<a href="{{list.url}}">
								...
								<div class="item-rating">
									<div class="rank">
                                        ...
                                        <span>{{list.rating == null ? '暂无评分' : list.rating.value | fixed}}</span>
									</div>
								</div>
							</a>
						</li>
            ...
            <section id="market_product_book">
                <div v-for="item in books.header" id="book_header">
                    <header>
                    <h2>豆瓣纸书</h2>
                    <a href="https://market.douban.com/book/">更多</a>
                    </header>
                    <a href="https://market.douban.com/book/baiguiyexing" class="promBook">
                    <img class="cover" v-lazy="item.cover.url" alt="百鬼夜行 阳">
                    <div class="content">
                        <div class="title">
                            <span class="price">¥ {{item.price}}</span>
                            <p class="name">{{item.title}}</p>
                        </div>
                        <p class="infoWithoutReviewer">{{item.info}}</p>
                    </div>
                    </a>
                </div>
                <section class="section-content">
                    <ul class="row items">
                        <li v-for="list in books.subject_collection_items" class="item">
                            <a href="{{list.url}}">
                                <div class="item-poster" :style="{backgroundImage:'url(' +list.cover.url+ ')'}">
                                </div>
                                <span class="item-title">{{list.title}}</span>
                                <div class="item-rating">
                                    <div class="rank">
            ...
            <section class="types">
                ...
                <div class="section-content">
                    <a name="cate"></a>
                    <ul class="type-list">
                         <li v-for="item in type"><a v-link="{ name:'list',params:{title:item.text,type:'book',id:item.lab || ''} }">{{item.text}}<span></span></a></li>
                    </ul>
                </div>
            </section>
